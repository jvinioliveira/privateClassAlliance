-- Slot mode by credit type:
-- 1) teacher no longer manages slot capacity manually
-- 2) slot capacity is standardized at 2
-- 3) booking allows a single active class per slot (individual or dupla)
...
SET capacity = 2
WHERE capacity IS DISTINCT FROM 2;

CREATE OR REPLACE FUNCTION public.enforce_auto_slot_capacity()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_enforce_auto_slot_capacity ON public.availability_slots;
CREATE TRIGGER trg_enforce_auto_slot_capacity
  BEFORE INSERT OR UPDATE ON public.availability_slots
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_auto_slot_capacity();
...
BEGIN
  ...
  IF v_slot.start_time <= now() THEN
    RAISE EXCEPTION 'Este horario esta indisponivel porque ja passou.';
  END IF;

  IF v_slot.start_time <= (now() + INTERVAL '30 minutes') THEN
    RAISE EXCEPTION 'Agendamento indisponivel com menos de 30 minutos de antecedencia.';
  END IF;
