...
type AdminPlansUiState = {
  selectedManagementClassType: PlanClassType | null;
  open: boolean;
  editingPlanId: string | null;
  form: {
    name: string;
    description: string;
    classType: PlanClassType;
    credits: number;
    priceReais: string;
    sortOrder: number;
    isActive: boolean;
...
const ADMIN_PLANS_UI_STATE_KEY = 'admin:plans:ui-state:v1';
...
const AdminPlansPage = () => {
  ...
  useEffect(() => {
    if (didRestoreStateRef.current || isLoading || plans.length === 0) return;

    const raw = window.sessionStorage.getItem(ADMIN_PLANS_UI_STATE_KEY);
    if (!raw) {
    ...
    try {
      const saved = JSON.parse(raw) as AdminPlansUiState;
      if (saved.selectedManagementClassType) {
        setSelectedManagementClassType(saved.selectedManagementClassType);
      }

      setName(saved.form.name ?? '');
      setDescription(saved.form.description ?? '');
      setClassType(saved.form.classType ?? 'individual');
      setCredits(Number.isFinite(saved.form.credits) ? saved.form.credits : 1);
      setPriceReais(saved.form.priceReais ?? '100.00');
      setSortOrder(Number.isFinite(saved.form.sortOrder) ? saved.form.sortOrder : 0);
      setIsActive(Boolean(saved.form.isActive));

      if (saved.editingPlanId) {
        const matchedPlan = plans.find((plan) => plan.id === saved.editingPlanId) ?? null;
        setEditingPlan(matchedPlan);
      } else {
        setEditingPlan(null);
      }
      ...
      window.sessionStorage.removeItem(ADMIN_PLANS_UI_STATE_KEY);
    } finally {
      didRestoreStateRef.current = true;
    }
  }, [isLoading, plans]);
  ...
  useEffect(() => {
    ...
    const payload: AdminPlansUiState = {
    ...
    window.sessionStorage.setItem(ADMIN_PLANS_UI_STATE_KEY, JSON.stringify(payload));
  }, [selectedManagementClassType, open, editingPlan?.id, name, description, classType, credits, priceReais, sortOrder, isActive]);
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {visiblePlans.map((plan) => (
                <div key={plan.id} className="animate-fade-in rounded-xl border border-border bg-card p-4">
            ...
            <div className="space-y-2">
              ...
              {editingPlan ? (
                <div className="rounded-lg border border-border/70 bg-background/60 p-3 text-sm">
                  <span className="font-medium text-foreground">{getClassTypeLabel(classType)}</span>
                  <p className="mt-1 text-xs text-muted-foreground">Categoria bloqueada após criação do plano.</p>
                </div>
              ) : (
